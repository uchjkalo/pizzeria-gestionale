...
// Classi CSS per urgenza - Light Theme pulito
const urgencyBorder = (m: number, u: boolean) =>
  u || m >= 20 ? "border-red-500 bg-red-50" : m >= 12 ? "border-red-400 bg-red-50/50" : "border-gray-400 bg-gray-100";

const urgencyBadge = (m: number, u: boolean) =>
  u || m >= 20 ? "bg-red-600 text-white" : m >= 12 ? "bg-red-600 text-white" : "bg-gray-200 text-gray-900";
...
export default function KitchenZone({ zone }: Props) {
  ...
  const OrderCard = ({ order }: { order: Order }) => {
    ...
    return (
      <div className={`rounded-2xl border p-5 transition-all shadow-sm hover:shadow-md ${urgencyBorder(minutes, order.isUrgent)}`}>
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex flex-wrap gap-2 flex-1">
            ...
            <span className={`px-3 py-1.5 rounded-full text-xs font-bold ${order.status === "attesa" ? "bg-gray-200 text-gray-900" : order.status === "preparazione" ? "bg-red-100 text-red-700" : "bg-red-100 text-red-700"}`}>
            ...
            {order.isPaid && <span className="bg-red-100 text-red-700 px-2.5 py-1 rounded-full text-xs font-bold">💳 Pagato</span>}
          </div>
        ...
        {order.desiredTime && <p className="text-red-700 text-sm mb-3 font-medium">🕐 Pronto per le {order.desiredTime}</p>}

        {/* Items */}
        <div className="space-y-2 mb-4">
          {relItems.map(item => (
            <div key={item.cartId} className={`rounded-lg p-3 border ${item.id === "custom_pizza" ? "bg-red-100 border-red-300" : "bg-gray-50 border-gray-300"}`}>
              <div className="flex items-center gap-2 flex-wrap mb-2">
                {item.id === "custom_pizza" && <span className="text-xs bg-red-600 text-white px-2 py-0.5 rounded font-bold">🎨 PERSONALIZZATA</span>}
                <span className={`font-bold text-base ${item.id === "custom_pizza" ? "text-red-700" : "text-gray-900"}`}>
                  {item.quantity > 1 && <span className="text-red-600">×{item.quantity} </span>}{item.name}
                </span>
                {item.size !== "normale" && <span className={`text-sm px-2 py-0.5 rounded font-bold ${item.size === "maxi" ? "bg-red-200 text-red-700" : "bg-gray-200 text-gray-900"}`}>{item.size.toUpperCase()}</span>}
              </div>
              {item.isHalf && item.halfPizza1 && item.halfPizza2 && <p className="text-red-700 text-sm font-bold mb-1">½ {item.halfPizza1.name} + ½ {item.halfPizza2.name}</p>}
              {item.removedIngredients.length > 0 && <p className="text-red-600 font-bold">🚫 SENZA: {item.removedIngredients.join(", ")}</p>}
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          ...
          {order.status === "pronto" && <div className="flex-1 text-center text-red-700 font-bold py-2.5 rounded-lg bg-red-50 border border-red-300">✅ Pronto!</div>}
          <button
            onClick={async () => {
            ...
            className="bg-gray-200 hover:bg-gray-300 text-gray-900 text-sm font-semibold py-2.5 px-4 rounded-lg transition-colors">
            📋 Auto-Task
          </button>
        </div>
        ...
        {isDeleting ? (
          <div className="flex gap-2 mt-2">
            ...
            <button onClick={() => setConfirmDelete(null)} className="flex-1 bg-gray-200 text-gray-900 text-sm py-2 rounded-lg hover:bg-gray-300">✗ Annulla</button>
          </div>
        ) : (
          <button onClick={() => setConfirmDelete(order.id)} className="w-full text-gray-600 hover:text-red-600 text-xs py-2 rounded-lg hover:bg-red-50 transition-colors font-medium">🗑 Elimina ordine</button>
        )}
      </div>
    );
  };
  ...
  const TaskPanel = () => (
    <div className="flex flex-col h-full overflow-hidden bg-gray-100">
      ...
      <div className="shrink-0 p-4 space-y-3 border-b border-gray-300">
        <div className="flex gap-2">
          <input value={newTask} onChange={e => setNewTask(e.target.value)}
            ...
            className="flex-1 bg-white text-gray-900 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-red-600 border border-gray-300 focus:border-red-600 placeholder-gray-500" />
        ...
        {/* Preset buttons */}
        <div>
          <p className="text-gray-700 text-[10px] uppercase tracking-widest font-bold mb-2">Quick Actions</p>
          <div className="flex flex-wrap gap-1.5">
            {PRESET_TASKS[zone].map(preset => (
              <button key={preset} onClick={() => handleCreateTask(preset)}
                className="bg-gray-200 hover:bg-gray-300 active:bg-red-200 text-gray-900 text-xs px-3 py-1.5 rounded-full border border-gray-400 transition-colors font-medium hover:border-red-500">
                {preset}
              </button>
            ))}
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {tasks.length === 0 && (
          <div className="text-center text-gray-600 mt-8"><p className="text-3xl mb-2">✅</p><p className="text-sm font-medium">Nessun task attivo</p></div>
        )}
        {tasks.map(task => (
          <div key={task.id} className="bg-gray-100 rounded-xl p-3 border border-gray-400 group hover:shadow-md transition-all">
            {editingTaskId === task.id ? (
              <div className="flex gap-2">
                <input autoFocus value={editingText} onChange={e => setEditingText(e.target.value)}
                  ...
                  className="flex-1 bg-white text-gray-900 rounded-lg px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-red-600 border border-gray-300" />
                <button onClick={handleSaveEdit} className="text-red-600 text-sm font-bold px-2">✓</button>
                <button onClick={() => setEditingTaskId(null)} className="text-gray-600 text-sm px-2">✗</button>
              </div>
            ) : (
              <div className="flex items-start gap-3">
                <button onClick={() => completeTask(task.id)}
                  className="w-6 h-6 rounded-full border-2 border-gray-400 hover:border-red-500 hover:bg-red-100 flex items-center justify-center shrink-0 mt-0.5 transition-all active:scale-90">
                  <span className="text-red-600 text-sm opacity-0 group-hover:opacity-100 font-bold">✓</span>
                </button>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm leading-snug font-medium ${task.description.startsWith("  ") ? "text-gray-600 text-xs pl-2" : "text-gray-900"}`}>
                    {task.description}
                  </p>
                ...
                <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditingTaskId(task.id); setEditingText(task.description); }}
                    className="w-6 h-6 text-gray-600 hover:text-red-600 flex items-center justify-center text-sm transition-colors">✏️</button>
                  <button onClick={() => deleteTask(task.id)}
                    className="w-6 h-6 text-gray-600 hover:text-red-600 flex items-center justify-center text-sm transition-colors">✕</button>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="border-t border-gray-300 shrink-0">
        <button onClick={() => setShowAll(s => !s)}
          className="w-full px-4 py-3 text-gray-700 text-sm flex items-center justify-between hover:text-gray-900 hover:bg-gray-200 transition-colors font-medium">
        ...
        {showAllTasks && (
          <div className="max-h-48 overflow-y-auto px-4 pb-4 space-y-1.5">
            {allTasks.map(t => (
              <div key={t.id} className={`flex items-center gap-2 rounded-lg px-3 py-2 transition-all ${t.completed ? "bg-gray-200/50" : "bg-gray-100 border border-gray-400"}`}>
                <span className={`text-sm shrink-0 font-semibold ${t.completed ? "text-red-600" : "text-gray-600"}`}>{t.completed ? "✓" : "○"}</span>
                {editingTaskId === t.id ? (
                  <div className="flex gap-2 flex-1">
                    <input autoFocus value={editingText} onChange={e => setEditingText(e.target.value)}
                      onKeyDown={e => { if (e.key === "Enter") handleSaveEdit(); if (e.key === "Escape") setEditingTaskId(null); }}
                      className="flex-1 bg-white text-gray-900 rounded-lg px-2 py-1 text-xs outline-none border border-gray-300 focus:ring-2 focus:ring-red-600" />
                    <button onClick={handleSaveEdit} className="text-red-600 text-xs font-bold">✓</button>
                    <button onClick={() => setEditingTaskId(null)} className="text-gray-600 text-xs">✗</button>
                  </div>
                ) : (
                  <>
                    <span className={`text-xs flex-1 ${t.completed ? "line-through text-gray-500" : "text-gray-900"}`}>{t.description}</span>
                    <span className="text-gray-500 text-[10px] shrink-0">{formatTime(t.createdAt)}</span>
                    <button onClick={() => { setEditingTaskId(t.id); setEditingText(t.description); }}
                      className="text-gray-600 hover:text-red-600 text-sm shrink-0 w-5 h-5 flex items-center justify-center transition-colors">✏️</button>
                    <button onClick={() => deleteTask(t.id)}
                      className="text-gray-600 hover:text-red-600 text-sm shrink-0 w-5 h-5 flex items-center justify-center transition-colors">✕</button>
                  </>
                )}
              </div>
            ))}
    ...
    <div className="h-[calc(100vh-100px)] md:h-[calc(100vh-80px)] flex flex-col bg-gray-50">
      ...
      <main className="flex-1 overflow-hidden flex flex-col md:flex-row gap-4 md:gap-5 p-4 md:p-6">
        ...
        <div className="hidden md:flex gap-5 flex-1 overflow-hidden">
          ...
          {/* Task Panel */}
          <div className="w-96 bg-gray-100 rounded-2xl border border-gray-400 flex flex-col overflow-hidden shadow-sm">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-300 shrink-0 bg-gray-100">
              <h2 className="text-gray-900 font-bold text-lg">📋 Task</h2>
              <span className={`text-xs font-bold px-3 py-1 rounded-full ${tasks.length > 0 ? "bg-red-600 text-white" : "bg-gray-200 text-gray-700"}`}>{tasks.length}</span>
            </div>
