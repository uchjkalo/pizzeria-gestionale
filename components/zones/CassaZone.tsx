...
const statusLabel = (s: string) => {
  switch (s) {
    case "attesa":       return { label: "⏳ In attesa",  color: "bg-gray-200 text-gray-900 border border-gray-400" };
    case "preparazione": return { label: "🔧 In prep.",   color: "bg-red-900/30 text-red-200 border border-red-700" };
    case "pronto":       return { label: "🔧 Rifinitura", color: "bg-red-900/40 text-red-200 border border-red-700" };
    case "consegnato":   return { label: "✅ Consegnato", color: "bg-gray-700/60 text-gray-200 border border-gray-500" };
    default:             return { label: s,               color: "bg-gray-200 text-gray-900 border border-gray-400" };
  }
};
...
function PayModal({ order, initialQuickExtras, onConfirm, onClose }: PayModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center"
      ...
      <div className="bg-gray-100 rounded-t-3xl sm:rounded-2xl w-full max-w-md flex flex-col border-t sm:border border-gray-400 shadow-2xl"
        ...
        {/* Header */}
        <div className="px-4 pt-3 pb-2 border-b border-gray-400 shrink-0 flex items-start justify-between">
          <div>
          ...
          <button onClick={() => onClose(quickExtras)}
            className="w-11 h-11 bg-gray-200 active:bg-gray-300 text-gray-900 rounded-2xl flex items-center justify-center text-2xl font-bold ml-2 shrink-0">
            ×
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          ...
          {view === "riepilogo" && (
            <div className="p-4 space-y-3">
              <div className="bg-gray-200 rounded-2xl px-4 py-3 space-y-2 border border-gray-400">
                ...
                {buildReceiptLines(order, []).map(line => (
                  <div key={line.key} className="flex items-baseline justify-between gap-2">
                    <div className="flex items-baseline gap-1.5 min-w-0 flex-1">
                      ...
                      {line.dim && <span className="text-[9px] font-black bg-red-900/50 text-red-200 px-1 rounded shrink-0">{line.dim}</span>}
                ...
                {quickExtras.length > 0 && (
                  <>
                    <div className="border-t border-dashed border-gray-400 mt-2 pt-2">
                      <p className="text-[9px] uppercase tracking-widest text-red-300 font-bold mb-2">Aggiunte post-ordine</p>
                      {quickExtras.map(e => (
                        <div key={e.id} className="flex items-center justify-between gap-2 mb-1.5">
                          <div className="flex items-center gap-1.5 flex-1 min-w-0">
                            <span className="text-red-300 text-[10px] font-mono shrink-0">1×</span>
                            <span className="text-red-200 text-sm font-semibold truncate">{e.label}</span>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <span className="text-red-200 text-sm font-mono tabular-nums">€{e.price.toFixed(2)}</span>
                            <button onClick={() => removeQuickExtra(e.id)}
                              className="w-7 h-7 bg-gray-300 hover:bg-red-900/40 text-gray-900 hover:text-red-200 rounded-lg flex items-center justify-center text-base transition-all">
                              ×
                            </button>
                          </div>
                        </div>
                ...
                {/* Totale */}
                <div className="border-t border-dashed border-gray-400 mt-2 pt-3 flex justify-between items-baseline">
                  <span className="text-gray-600 text-xs uppercase tracking-widest">Totale</span>
                  <span className="text-red-400 font-black text-2xl tabular-nums">€{grandTotal.toFixed(2)}</span>
                </div>
              </div>

              {order.orderNotes && (
                <div className="bg-red-900/30 border border-red-700 rounded-xl px-3 py-2">
                  <p className="text-red-200 text-xs">📋 {order.orderNotes}</p>
                </div>
              )}

              <button onClick={() => setView("aggiunte")}
                className="w-full bg-gray-200 border border-gray-400 text-gray-900 text-sm py-3 rounded-2xl font-semibold hover:bg-gray-300 transition-colors">
                ➕ Aggiungi al conto
              </button>
              <button onClick={() => setView("pagamento")}
                className="w-full bg-gradient-to-r from-red-600 to-red-500 text-white font-bold py-4 rounded-2xl text-sm shadow-[0_4px_16px_rgba(194,38,46,0.35)]">
            ...
            <div className="p-4 space-y-4">
                  ...
                  {QUICK_ITEMS.map(qi => (
                    <div key={qi.label} className="flex flex-col gap-1">
                      <button onClick={() => addQuickItem(qi)}
                        className="bg-gray-200 active:bg-red-900/30 border border-gray-400 text-gray-900 rounded-xl py-2.5 text-xs font-semibold text-center transition-all hover:bg-gray-300">
                        <div className="text-lg mb-0.5">{qi.emoji}</div>
                        {qi.label}
                      </button>
                      <div className="flex items-center bg-gray-100 border border-gray-400 rounded-lg px-1.5 py-1 gap-1">
              ...
              <div>
                <p className="text-gray-600 text-[10px] uppercase tracking-widest font-bold mb-2">Personalizzata</p>
                 <div className="bg-gray-200 border border-gray-400 rounded-2xl p-3 space-y-2">
                  <input type="text" value={customLabel} onChange={e => setCustomLabel(e.target.value)}
                    placeholder="Descrizione (opzionale)"
                    className="w-full bg-gray-100 text-gray-900 rounded-xl px-3 py-2.5 text-sm outline-none ring-1 ring-gray-400 focus:ring-2 focus:ring-red-600 placeholder:text-gray-600" />
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input type="number" step={0.5} value={customPrice}
                        ...
                        placeholder="0.00"
                        className="w-full bg-gray-100 text-gray-900 rounded-xl pl-7 pr-3 py-2.5 text-sm outline-none ring-1 ring-gray-400 focus:ring-2 focus:ring-red-600 placeholder:text-gray-600 tabular-nums" />
                    </div>
              ...
              {quickExtras.length > 0 && (
                <div>
                  <p className="text-red-300 text-[10px] uppercase tracking-widest font-bold mb-2">Aggiunte post-ordine</p>
                  <div className="space-y-1.5">
                    {quickExtras.map(e => (
                      <div key={e.id} className="flex items-center justify-between bg-red-900/30 border border-red-700/50 rounded-xl px-3 py-2.5">
                        <span className="text-red-200 text-sm font-semibold">{e.label}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-red-200 font-bold tabular-nums">€{e.price.toFixed(2)}</span>
                          <button onClick={() => removeQuickExtra(e.id)}
                            className="w-8 h-8 bg-gray-300 active:bg-red-900/40 text-gray-900 rounded-xl flex items-center justify-center text-lg transition-all hover:bg-red-900/40">
                            ×
                          </button>
                        </div>
                      </div>
                    ))}
                    <div className="flex justify-between text-xs text-red-200 px-1 pt-1">
              ...
              <button onClick={() => setView("riepilogo")}
                className="w-full bg-gray-200 border border-gray-400 text-gray-900 py-3 rounded-2xl text-sm font-semibold hover:bg-gray-300 transition-colors">
          ...
          {view === "pagamento" && (
            <div className="p-4 space-y-4">
              {/* Totale recap */}
              <div className="bg-gray-200 border border-gray-400 rounded-2xl px-4 py-3 flex justify-between items-center">
                <span className="text-gray-800 text-sm">Da incassare</span>
                <span className="text-red-400 font-black text-3xl tabular-nums">€{grandTotal.toFixed(2)}</span>
              </div>
                    ...
                    <button key={m} onClick={() => setMethod(m)}
                      className={`py-4 rounded-2xl font-bold text-sm border-2 transition-all ${
                        method === m
                          ...
                          : "border-gray-400 bg-gray-200 text-gray-900 hover:bg-gray-300"
                      }`}>
              ...
              {method === "contanti" && (
                <div>
                    <p className="text-gray-700 text-[10px] uppercase tracking-widest font-bold mb-2">Calcola resto</p>
                    <div className="flex gap-2 items-center">
                      <div className="relative flex-1">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-700 font-bold">€</span>
                        <input type="number" min={0} step={0.5} value={cash}
                          onChange={e => setCash(e.target.value)} placeholder="Importo ricevuto"
                          className="w-full bg-gray-100 text-gray-900 rounded-xl pl-7 pr-3 py-3 outline-none ring-1 ring-gray-400 focus:ring-2 focus:ring-red-600 text-sm" />
                      </div>
                    {change !== null && (
                      <div className={`text-right shrink-0 ${change < 0 ? "text-red-400" : "text-gray-900"}`}>
                  ...
                  <div className="flex gap-2 mt-2">
                      {[5, 10, 20, 50].map(v => (
                        <button key={v} onClick={() => setCash(String(v))}
                          className="flex-1 bg-gray-300 border border-gray-400 text-gray-900 text-sm py-2 rounded-xl font-semibold">
                        €{v}
                      </button>
                    ))}
                  </div>
              ...
              <button onClick={() => setFelice(f => !f)}
                className={`w-full py-2.5 rounded-xl text-sm border transition-all ${
                  felice ? "border-red-600 bg-red-900/40 text-red-200" : "border-gray-400 bg-gray-200 text-gray-900"
                }`}>
                {felice ? "😊" : "😐"}
              </button>
            </div>
        ...
        {view === "pagamento" && (
          <div className="border-t border-gray-400 p-4 shrink-0 space-y-2 bg-gray-100">
            <button onClick={() => onConfirm(method, felice, quickExtras)}
              className="w-full bg-gradient-to-r from-red-600 to-red-500 text-white font-black py-4 rounded-2xl text-base shadow-[0_4px_20px_rgba(194,38,46,0.35)] active:scale-[0.98] transition-all hover:from-red-700 hover:to-red-600">
    ...
    <>
      ...
      <div className="h-[calc(100vh-80px)] flex flex-col gap-3 overflow-hidden">
        ...
        <div className="flex items-center justify-between shrink-0 flex-wrap gap-3">
          <h1 className="text-gray-900 text-xl font-bold">💳 Cassa</h1>
          <div className="flex gap-2">
            <div className="bg-gray-100 border border-gray-400 rounded-2xl px-4 py-2 text-right shadow-sm">
              <p className="text-gray-700 text-[10px] uppercase tracking-wider">Da incassare</p>
              <p className="text-red-400 text-xl font-black tabular-nums">€{totaleDaPagare.toFixed(2)}</p>
            ...
            <div className="bg-gray-100 border border-gray-400 rounded-2xl px-4 py-2 text-right shadow-sm">
              ...
              <p className="text-red-300 text-xl font-black tabular-nums">€{totaleIncassato.toFixed(2)}</p>
        ...
        <div className="flex gap-2 shrink-0">
          <button onClick={() => setTab("daPagare")}
            className={`px-4 py-2.5 rounded-xl font-bold text-sm transition-colors border ${tab === "daPagare" ? "border-red-600 bg-red-900/40 text-red-200" : "border-gray-400 bg-gray-200 text-gray-900 hover:bg-gray-300"}`}>
            💰 Da pagare ({daPagare.length})
          </button>
          <button onClick={() => setTab("pagati")}
            className={`px-4 py-2.5 rounded-xl font-bold text-sm transition-colors border ${tab === "pagati" ? "border-red-700 bg-red-900/30 text-red-200" : "border-gray-400 bg-gray-200 text-gray-900 hover:bg-gray-300"}`}>
              ...
              return (
                <div key={order.id}
                  className={`bg-gray-100 rounded-2xl border p-4 flex flex-col gap-3 shadow-sm ${
                    order.isPaid ? "border-red-700" :
                    order.status === "pronto" ? "border-red-600" :
                    hasSavedExtras ? "border-red-500" : "border-gray-400"
                  }`}>

                  {/* Header */}
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-gray-900 font-bold text-sm">{orderTypeLabel(order)}</p>
                    ...
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      ...
                      {order.isPaid && <span className="text-xs bg-red-900/40 text-red-200 px-2 py-0.5 rounded-lg font-bold border border-red-700">✅ PAGATO</span>}
                  ...
                  {/* Receipt compatto */}
                  <div className="bg-gray-200 rounded-xl px-3 py-2.5 space-y-1.5 border border-gray-400">
                    {/* Righe ordine */}
                    {buildReceiptLines(order, []).map(line => (
                      <div key={line.key} className="flex items-baseline justify-between gap-2">
                        <div className="flex items-baseline gap-1.5 min-w-0 flex-1">
                    ...
                    {(savedExtras[order.id]?.length ?? 0) > 0 && (
                      <div className="border-t border-dashed border-gray-400 pt-1.5 mt-1">
                        <p className="text-[9px] text-red-300 uppercase tracking-widest font-bold mb-1">Post-ordine</p>
                        {savedExtras[order.id]!.map(e => (
                          <div key={e.id} className="flex items-baseline justify-between gap-2">
                            <div className="flex items-baseline gap-1.5 flex-1 min-w-0">
                              <span className="text-red-300 text-[10px] font-mono shrink-0">1×</span>
                              <span className="text-red-200 text-xs truncate">{e.label}</span>
                            </div>
                            <span className="text-red-200 text-xs font-mono tabular-nums shrink-0">€{e.price.toFixed(2)}</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  {/* Totale + azione */}
                  <div className="flex items-center justify-between pt-2 border-t border-gray-400 mt-auto">
                    <span className="text-red-400 text-2xl font-black tabular-nums">€{cardTotal.toFixed(2)}</span>
                    {order.isPaid ? (
                      <div className="text-right">
                        <p className="text-red-200 text-sm font-bold">{order.paymentMethod === "contanti" ? "💵 Contanti" : "💳 Carta"}</p>
                        {order.paidAt && <p className="text-gray-400 text-xs">{formatTime(order.paidAt)}</p>}
                      </div>
                    ) : (
                      <button onClick={() => setPayingOrderId(order.id)}
                        className="bg-gradient-to-r from-red-600 to-red-500 text-white font-bold px-4 py-2.5 rounded-xl text-sm shadow-[0_4px_12px_rgba(194,38,46,0.3)] hover:from-red-700 hover:to-red-600 transition-all">
                  ...
                  {order.isPaid && (
                    isReverting ? (
                      <div className="flex gap-2">
                        <button onClick={() => handleRevertPayment(order.id)} className="flex-1 bg-red-600 text-white text-xs font-bold py-2.5 rounded-xl hover:bg-red-700 transition-colors">↩ Sì, annulla</button>
                      ...
                      <button onClick={() => setConfirmRevert(order.id)}
                        className="w-full bg-gray-200 text-gray-900 text-xs py-2 rounded-xl border border-gray-400 hover:text-red-300 hover:border-red-600 transition-colors">
                        ↩ Annulla pagamento
                      </button>
                    )
                  )}
                  ...
                  {isDeleting ? (
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setConfirmDelete(order.id)}
                        className="w-full text-gray-700 text-xs py-1.5 hover:text-red-300 transition-colors rounded-lg hover:bg-red-900/20">
                        🗑 Elimina ordine
                      </button>
                    )}
                </div>
