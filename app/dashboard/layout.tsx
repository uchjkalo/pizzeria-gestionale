...
const zones = [
  ...
  { name: "Preparaz.",    path: "/dashboard/preparazione", emoji: "🍕" },  // task prep a freddo
  { name: "Rifinitura",   path: "/dashboard/rifinitura",   emoji: "📦" },  // assembly + consegna
        ...
        {zones.map(z => (
          <Link key={z.path} href={z.path}
            className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
              pathname === z.path ? "bg-orange-500 text-white" : "text-gray-300 hover:bg-gray-700"
            }`}>
            {z.emoji} {z.name}
          </Link>
        ))}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-gray-800 border-t border-gray-700 z-40"
        ...
        <div className="grid grid-cols-7 h-16">
          {zones.map(z => {
            ...
            return (
              <Link key={z.path} href={z.path}
                className={`flex flex-col items-center justify-center gap-0.5 transition-colors ${active ? "text-orange-400" : "text-gray-500"}`}>
                <span className="text-2xl leading-none">{z.emoji}</span>
                <span className="text-[9px] font-medium leading-none truncate w-full text-center px-0.5">{z.name}</span>
              </Link>
            );
          })}
        </div>
